...
def get_project_id(owner, project_number, token):
    ...
    for query_type, query in [("user", user_query), ("organization", org_query)]:
        ...
        if response.status_code == 200:
            ...
            if ("data" in data and 
                data["data"][query_type] and 
                data["data"][query_type]["projectV2"]):
...
def main():
    ...
    print(f"2. Set up priority-based views using PRIORITY_SETUP_GUIDE.md")
    print(f"3. Move issues to appropriate columns (Backlog, In Progress, etc.)")
