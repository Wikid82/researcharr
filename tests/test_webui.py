...
def test_user_settings_save_password_change(client):
    login(client)
    rv = client.post('/user', data={'username': 'admin', 'password': 'newpass'}, follow_redirects=True)
...
def test_scheduling_save_invalid(client):
    ...
    with client.session_transaction() as sess:
        sess['logged_in'] = True
    rv = client.post('/scheduling', data={
        'cron_schedule': '',  # blank to trigger error
        'timezone': 'UTC'
    }, follow_redirects=True)
...
def test_radarr_settings_save_invalid(client):
    ...
    with client.session_transaction() as sess:
        sess['logged_in'] = True
    rv = client.post('/settings/radarr', data={
        'radarr0_enabled': 'on',
        'radarr0_name': '',
        'radarr0_url': '',
        'radarr0_api_key': '',
...
def test_sonarr_settings_save_invalid(client):
    ...
    with client.session_transaction() as sess:
        sess['logged_in'] = True
    rv = client.post('/settings/sonarr', data={
        'sonarr0_enabled': 'on',
        'sonarr0_name': '',
        'sonarr0_url': '',
        'sonarr0_api_key': '',
