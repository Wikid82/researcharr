...
def countdown(minutes, logger=None):
    import sys, time
    if logger:
        logger.info(f"Countdown: {minutes} minutes until next scheduled run (test mode)")
    ...
    sys.stdout.write("\n")
    if logger:
        logger.info("Countdown complete. If this is a test, the next cron run should now occur.")
