...
def patch_config_paths(tmp_path_factory, monkeypatch):
    ...
    monkeypatch.setattr("researcharr.app.DB_PATH", str(temp_dir / "researcharr.db"), raising=False)
    ...
    monkeypatch.setattr("researcharr.app.setup_logger", lambda name, log_file, level=None: mock.Mock(), raising=False)
...
def test_radarr_connection_unreachable(monkeypatch):
    from researcharr import app
    logger = mock.Mock()
    with mock.patch("researcharr.app.requests.get", side_effect=Exception("unreachable")) as mock_get:
...
def test_init_db_idempotent(tmp_path, monkeypatch):
    ...
    conn = sqlite3.connect(str(db_path))
    cursor = conn.cursor()
    cursor.execute("INSERT INTO radarr_queue (movie_id, last_processed) VALUES (?, ?)", (1, "2025-10-22"))
    conn.commit()
    conn.close()
...
def test_insert_and_retrieve_radarr_queue(tmp_path, monkeypatch):
    ...
    conn = sqlite3.connect(str(db_path))
    cursor = conn.cursor()
    cursor.execute("INSERT INTO radarr_queue (movie_id, last_processed) VALUES (?, ?)", (42, "2025-10-22"))
    conn.commit()
...
def test_insert_and_retrieve_sonarr_queue(tmp_path, monkeypatch):
    ...
    conn = sqlite3.connect(str(db_path))
    cursor = conn.cursor()
    cursor.execute("INSERT INTO sonarr_queue (episode_id, last_processed) VALUES (?, ?)", (99, "2025-10-22"))
    conn.commit()
