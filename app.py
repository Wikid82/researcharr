...
def setup_logger(name, log_file, level=logging.INFO):
    ...
    handler.setFormatter(
        logging.Formatter(
            "%(asctime)s %(message)s [%(levelname)s] [%(name)s] [%(process)d] [%(thread)d] [%(timezone)s]",
...
def load_config(path=None):
    """Loads and returns the YAML config as a dict. Accepts optional path for testability."""
...
def countdown(minutes, logger=None):
    ...
    if logger:
        logger.info(
            f"Countdown: {minutes} minutes until next scheduled run (test mode)"
        )
    ...
    if logger:
        logger.info(
            "Countdown complete. If this is a test, the next cron run should now occur."
        )
...
def main():
    ...
    if not has_valid_url_and_key(radarr_instances) and not has_valid_url_and_key(
        ...
        main_logger.warning(
            "No enabled Radarr or Sonarr instance has a valid URL (must start with http:// or https://) and API key. Please update your config.yml using the web UI. The app will not run jobs until this is fixed."
        )
        print(
            "WARNING: No enabled Radarr or Sonarr instance has a valid URL (must start with http:// or https://) and API key. Please update your config.yml using the web UI. The app will not run jobs until this is fixed."
        )
